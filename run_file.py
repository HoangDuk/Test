...
def Tabu_search(init_solution, tabu_tenure, CC, first_time, Data1, index_consider_elite_set, delta, alpha, end_segment, data_set, number_of_cities):
    solution_pack_len = 0
    solution_pack = []
...
def Tabu_search_for_CVRP(CC, delta, alpha, end_segment, data_set, number_of_cities):
    Data1 = []
    list_init = []
    ...
    best_sol, best_fitness, result_print, solution_pack, Data1 = Tabu_search(init_solution=current_sol, tabu_tenure=Data.number_of_cities-1, CC=CC, first_time=True, Data1=Data1, index_consider_elite_set=0, delta=delta, alpha=alpha, end_segment=end_segment, data_set=data_set, number_of_cities=number_of_cities)
...
def run_simulation(params):
    number_of_cities, delta, alpha, end_segment, data_set = params
    ITE = 10
    # Đọc file dữ liệu
    ...
    for txt_file in txt_files:
        with open(txt_file, 'r') as file:
            ...
            for i in range(ITE):  # Giả sử ITE là 10
                BEST = []
                start_time = time.time()
                best_fitness, best_sol = Tabu_search_for_CVRP(1, delta, alpha, end_segment, data_set, number_of_cities)
                print("---------", txt_file, "---------")
                end_time = time.time()
    ...
    # Lưu workbook
    workbook.save(f"Result/Random_{data_set}_{number_of_cities}_{delta}_{alpha}_{end_segment}_CL1.xlsx")
    workbook.close()
...
def main():
    ...
    combinations = list(product(number_of_cities_options, delta_options, alpha_options, end_segment_options, data_set_options))
